...
func spawn_new_packet(packet_type, start_node, end_node):
	var packet_scene = get_packet_scene_from_packet_type(packet_type)
	var packet = _initialise_new_packet(start_node, end_node, packet_scene)
	packet_container.add_child(packet)
	return packet

func _initialise_new_packet(start_node, end_node, packet_scene):
	var packet = packet_scene.instantiate()
	packet.id = _initialize_id();
...
func _initialize_id():
	...
	return str(base_id) + "_" + str(random_suffix)
